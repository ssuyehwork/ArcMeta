...
[2026-07-21]
- **任务描述**: 托管文件夹监听与任务控制逻辑校准与追踪优化 (Plan-102)。
- **修改文件**:
    - **修改**: `src/util/ImportHelper.h/.cpp` (扩展 `importPaths` 接口支持静默后台入库模式，并返回 `QFuture` 以供状态追踪)
    - **修改**: `src/core/AutoImportManager.h/.cpp` (纠正入库接口调用；引入 `QFutureWatcher` 实现任务完成信号的精准发射，确保 UI 转圈动画与真实后台进度同步)
    - **修改**: `src/ui/MainWindow.cpp` (校准盘符按钮状态机切换逻辑；补全右键菜单分流逻辑并应用 `UiHelper::applyMenuStyle` 样式规范)
- **修改原因**: 消除“货不对板”实现，解决 UI 状态与后台任务脱节的竞态问题，确保自动化入库逻辑完全符合需求规格。
- **优化点**:
    - **状态机闭环**: 通过异步追踪机制，确保盘符按钮在任务队列真正清空后才停止转圈。
    - **静默后台化**: 自动入库不再触发进度对话框，实现无感物理同步。
    - **调试诊断**: 在 `AutoImportManager`、`MftReader`、`UsnWatcher` 和 `ImportHelper` 中注入全链路深度调试日志，覆盖 USN 底层捕获、MFT 信号发射、路径前缀判定、任务启动与完成全流程，用于精准定位 `ArcMeta.Library` 内部文件入库失效的具体环节。
    - **样式对齐**: 规范了右键菜单的图标与视觉风格。

[2026-07-20]
- **任务描述**: 数据库文件名盘符自适应命名 (Plan-97) 与磁盘管理栏 UI 集成 (Plan-99)。
- **修改文件**:
...
- **优化点**:
    - **自适应重命名**: 实现 Arcmeta_[SERIAL]_[LETTER].db 格式自动更名，支持运行时盘符漂移热迁移。
