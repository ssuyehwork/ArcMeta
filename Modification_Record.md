...
[2026-07-20]
- **任务描述**: 数据库文件名添加盘符后缀及自适应重命名方案 (Plan-97)。
- **修改文件**:
    - **修改**: src/meta/DatabaseManager.h/.cpp (更新 getMemoryDb 接口，实现基于盘符后缀的自适应重命名与冲突处理逻辑)
    - **修改**: src/meta/MetadataManager.cpp (扫描启动时引入正则解析；在路径操作中同步传递盘符信息)
    - **修改**: src/core/AutoImportManager.cpp (在自动入库流程中加入盘符感知逻辑)
- **修改原因**:
    1. **直观性**: 让用户能通过数据库文件名直接识别对应的物理磁盘。
    2. **漂移自适应**: 解决移动硬盘在不同挂载点（盘符）变化时，数据库文件名无法及时反映物理位置的问题。
- **优化点**:
    - **纠偏算法**: 数据库加载时自动检测并迁移旧文件名到新的盘符后缀格式。
    - **冲突保护**: 发现同一序列号存在多个后缀文件时，自动保留最新修改的，并将其余文件重命名为 _无效.db 备份，防止数据覆盖。
    - **正则解析**: 启动扫描支持 Arcmeta_SERIAL_LETTER.db 与 Arcmeta_SERIAL.db 混合共存。

[2026-07-20 补丁]
- **任务描述**: 修复数据库重命名方案未触达旧文件的问题。
- **修改文件**:
    - **修改**: src/meta/MetadataManager.cpp (启动扫描时注入磁盘在线映射，主动诱导重命名)
    - **修改**: src/meta/DatabaseManager.cpp (实现运行时的盘符漂移重命名迁移逻辑)
- **修改原因**:
    1. 之前的方案仅在文件加载时探测，若启动扫描未传盘符则不会触发重命名。
    2. 缺乏运行时的动态迁移机制。
- **优化点**:
    - **启动强制校准**: 启动时自动关联在线磁盘，将无后缀库强制迁移至当前盘符后缀。
    - **热迁移支持**: 即使在程序运行期间盘符发生变动，也能安全平滑地迁移物理数据库文件。

[2026-07-20 补丁2]
- **修改文件**: src/meta/DatabaseManager.cpp
- **修改原因**: 修复编译器警告 C4456 (隐藏上一个本地声明)。

[2026-07-20 补丁3]
- **任务描述**: 数据库重命名方案深度优化 (Plan-97 终版)。
- **修改文件**:
    - **修改**: src/meta/DatabaseManager.h/.cpp (重构锁机制，实现 getMemoryDbInternal；增强 init 方法以主动探测物理驱动器)
    - **修改**: src/meta/MetadataManager.cpp (优化初始化逻辑为两阶段加载：物理感知加载 + 文件兜底加载)
- **修改原因**:
    1. 彻底解决旧数据库文件在启动时因缺乏物理感知而无法自动更名的问题。
    2. 预防由物理探测导致的递归锁死锁。
- **优化点**:
    - **主动探测**: 启动时遍历所有 NTFS 驱动器，通过物理序列号匹配自动更正对应的数据库文件名。
    - **两阶段加载**: 确保所有在线磁盘优先以正确命名加载，离线磁盘以文件名为准兜底加载。
    - **安全备份**: 将冲突文件重命名为含时间戳的“无效”格式，并输出清晰的日志记录。

[2026-07-20 补丁4]
- **任务描述**: 盘符管理栏 UI 迁移 (Plan-98)。
- **修改文件**:
    - **修改**: src/ui/MainWindow.h/.cpp (新增 m_btnToggleDrives, m_driveBarWidget 及其显隐控制逻辑)
- **修改原因**: 提升多卷管理能力，迁移旧版本的盘符管理栏基础框架。
- **优化点**:
    - **布局规范**: 盘符管理栏固定高度 45px，背景色 #252526，底部边框 1px #333333，位于标题栏与导航栏之间。
    - **交互同步**: 标题栏新增切换按钮，支持 chevrons_up/down 图标动态联动。
