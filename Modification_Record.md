...
## 2026-11-14
- **任务描述**: 「空格键」快速预览画质优化、样式对齐及属性过滤 (Plan-109)。
- **修改文件**:
    - **修改**: `src/ui/QuickLookWindow.cpp` (区分标准图像渲染链路，重构滚动条样式)
    - **修改**: `src/ui/ContentPanel.cpp` (增加预览前置属性拦截)
    - **修改**: `src/ui/UiHelper.h` (新增 `isStandardImage` 判定)
- **修改原因**: 解决预览高清图片时的锯齿感、滚动条样式不规范以及非预览项（文件夹/exe/压缩包）被错误触发的问题。
- **优化点**:
    - **画质升级**: 针对标准图像（jpg/png/webp 等）绕过缩略图引擎，直接加载全分辨率原图并启用平滑变换。
    - **规范对齐**: 文本预览区滚动条废除硬编码 CSS，对齐 `Memories.md` 的 10px 宽度、3px 圆角规范。
    - **交互纯净**: 在信号端拦截文件夹、可执行文件及压缩包的预览请求，确保预览体验的一致性。
