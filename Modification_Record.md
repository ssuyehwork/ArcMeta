...
- **修改文件**:
    - **修改**: `src/ui/NavPanel.h/.cpp` (引入 `QSplitter` 拆分上下区，新增 `m_favoriteView` 及其持久化逻辑)
...
- **修改原因**: 提升大规模分类体系下的定位效率，统一侧边栏本地化交互标准。
- **优化点**:
    - **1:1 标准复刻**: 搜索框使用 `select` 图标，启用原生清除按钮，严格执行 6px 圆角与 `#2D2D2D` 背景规范。
    - **搜索豁免权**: 优化过滤逻辑，使系统项（ID < 0）和分组标题始终常驻，解决了父节点遮蔽导致搜索结果为空的问题。
    - **递归过滤引擎**: 实现基于 `NameRole` 的树形递归匹配，自动排除 `(n)` 计数后缀的干扰。
    - **视觉规范**: 使用亮蓝色 (#41F2F2) 执行关键词着色，增强视觉识别度。
    - **交互稳健性**: 补全了必要头文件并精细化处理了代理索引映射逻辑，确保 CRUD 操作安全。
