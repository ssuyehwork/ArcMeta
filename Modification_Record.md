...
[2026-11-16 11:30:00]
- **任务描述**: 规范化“扫描入库”右键菜单职责，实现物理 Move 与逻辑登记的职责分离。
- **修改文件**:
    - **修改**: `src/ui/ContentPanel.cpp` (重构 `ActionAddToCategory` 处理逻辑)
- **修改原因**: 解决右键菜单错误调用 `ImportHelper::importPaths` 的问题，确保菜单仅负责物理迁移，由 USN Journal 异步触发登记，维持托管库入口的唯一性。
- **优化点**:
    - **职责归位**: “扫描入库”动作现在仅执行物理移动，不再越权干涉入库流程。
    - **跨盘保护**: 增加物理卷校验，防止跨盘移动导致的数据损坏或耗时操作，并通过 `ToolTipOverlay` 提供错误反馈。
    - **自动初始化**: 移动时若目标托管库（`ArcMeta.Library_X`）不存在，则自动执行物理创建。
