...
- **优化点**:
    ...
    - **清理冗余**: 移除了已弃用的 `tagFilterText` 成员及其相关逻辑，修复了由此导致的编译错误。
    - **消除警告**: 使用 `Q_UNUSED` 处理了 `FilterPanel.cpp` 中的未引用形参警告。
...
[2026-06-22 05:20:00]
- **任务描述**: 排序相关 SVG 图标资源扩充。
- **修改文件**:
    - **修改**: `src/ui/SvgIcons.h` (新增 4 个排序图标)
- **修改原因**: 补充 UI 所需的排序状态反馈图标，支持升序/降序的视觉区分。
- **优化点**:
    - **资源精简**: 移除了原始 SVG 中的冗余 XML 标签与 Sketch 元数据。
    - **样式适配**: 统一采用 `currentColor` 填充，确保图标颜色能随 QSS 主题自动变化。

[2026-06-22 05:40:00]
- **任务描述**: 筛选器性能重构、逻辑统一与日期排序增强 (Plan-89/90/91)。
- **修改文件**:
    - **修改**: `src/ui/FilterPanel.h/.cpp` (实现 UI 复用池、移除标签区块、新增日期排序)
    - **修改**: `src/ui/ContentPanel.cpp` (FilterProxyModel 逻辑合并与关键词整合)
    - **修改**: `src/ui/MainWindow.cpp` (信号链整合与 doSearch 逻辑简化)
- **修改原因**: 解决大数据量下筛选面板重建导致的 UI 假死，统一搜索与过滤的双轨逻辑，并解决日期定位效率低下的问题。
- **优化点**:
    - **UI 复用池**: 引入 `m_rowPool` 管理筛选行控件，将“物理销毁”重构为“逻辑复用”，极大降低了主线程渲染负载。
    - **单轨过滤**: 将搜索词 `keyword` 彻底并入 `FilterState`，实现了搜索与过滤状态的透明同步。
    - **职责归口**: 移除筛选器内的标签区块，将多维搜索能力完全交付给已具备标签匹配能力的全局搜索框。
    - **交互增强**: 为日期筛选新增了 16x16px 的升降序按钮，并实现了独立的日期列表重排逻辑。
