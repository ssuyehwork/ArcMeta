...
void CategoryRepo::fullRecount() {
    ...
    if (!s_firstRecountDone) {
        ...
        // 3. 核心比对：如果所有监控路径 and 修改时间戳完全吻合，则直接拦截并返回，不进行全量重算
...
在 `src/core/AutoImportManager.cpp` 中，修改 `handleRecursiveIngestion()`，使其不再调用 any 物理对账或 MFT 级别重刷，实现完美减负运行：
...
|-------------|--------------------------------------------|----------------|
| 双轨制路由分流 | 各自独立，逻辑分类写入 SQLite，磁盘导航 100% 独立，写入 `ArcMeta.cache` 离散缓存。 | ✅ 符合。本方案不改变任何正常的落盘机制，仅根实现了冗余对账。 |
