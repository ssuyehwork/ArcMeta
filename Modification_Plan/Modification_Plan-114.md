...
## 1. 任务背景
本分析方案承接自 `Development_Plan.md` 中关于【NativeFolderWatcher 代码架构合理性、职责单一性与傻逼逻辑地毯式排查】的需求。由于系统底层核心监控服务 `NativeFolderWatcher` 承担着 Windows 文件系统底层高密集、异步、高并发的文件变动感知任务，其代码质量、事件吞吐机制、异常自愈及多线程生命周期设计直接决定了全款应用的运行稳定性、系统响应度（GUI 是否假死/卡顿）及元数据精确度。在此，我们将针对其代码架构、职责边界、事件防抖配对流程及潜在死锁、溢出或时序竞态等，展开细致、全面的地毯式排查并输出极致的模块化解耦重构图纸。

## 2. 问题定位

通过对 `src/core/NativeFolderWatcher.h` 与 `src/core/NativeFolderWatcher.cpp` 展开逐行深度审计，我们排查到了以下核心架构缺陷与不合理逻辑设计（“傻逼逻辑架构”）：
...
1. **精确依赖原则**：后续若要重构代码，在 `NativeFolderWatcher.h` 中必须引入 `#include <QList>`，以支持 `filesChanged` 批次信号的打包投递，并使用前置声明隔离业务依赖。
