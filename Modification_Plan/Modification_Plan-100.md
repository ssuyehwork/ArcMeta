# 侧边栏分类树状展开状态重启持久化失效根治重构 —— Modification_Plan-100.md
...
1. **信号在重置（Reset）期间发生回流**：
   In `CategoryPanel::initUi()`, the tree expanded/collapsed signals are connected to saveExpandedStateToSettings:
   ```cpp
   ...
   ```
   When `m_categoryModel->refresh()` is called to rebuild the tree, `beginResetModel()` and `endResetModel()` are executed, which uses `removeRows(0, rowCount())` to clear all rows.
   During node clearance, `QTreeView` triggers multiple synchronous `collapsed` signals because the nodes are physically removed.
   At this point, `saveExpandedStateToSettings` is synchronously callbacked. Since the guard lock `m_isInternalUpdating` is not set during model reset, the filter `if (m_isRestoringState || m_isInternalUpdating)` fails to intercept the signals.
   As nodes are already cleared, `saveExpandedState` captures an empty list of IDs and Names, which is written to `AppConfig` and physically written to disk.

2. **临时属性与磁盘配置状态不一致（表面正常，重启复现）**：
   Before reset, `modelAboutToBeReset` stores the correct state in tree properties.
   And in `modelReset` it is restored via `restoreExpandedState`.
   This makes the UI look expanded during execution, but in reality, the `AppConfig` on disk is already overwritten with an empty list.
   Once the user restarts, it reads the empty list and collapses all nodes (matching user's feedback).

3. **析构生命周期中的信号泄露**：
   When the user closes the app, `QTreeView` is garbage collected/unloaded, which also triggers large-scale `collapsed` signals, overwriting normal configuration with empty state.

## 3. 强制对照表

| 编号 | 用户原话 / 我的理解 | 方案对应点 | 是否一致 |
|------|---------------------|------------|----------|
| 1    | 侧边栏某个分类展开之后能够持续化，即便重启主程序之后仍然处于展开状态 (对应用户原话) | 彻底解决分类树展开持久化在模型重置与关闭时的异常覆写问题 (对应用户原话)。 | ✅ 一致 |
| 2    | 当我重启之后又回到折叠状态了，所以我期望你帮我扩大范围排查根本原因 (对应用户原话) | 扩大排查范围定位到 `beginResetModel` -> `removeRows` 产生的 collapsed 信号泄露这一底层竞态根源，并实施全生命周期安全加锁重构 (对应用户原话)。 | ✅ 一致 |
...
### 4.3 析构保护与信号安全切断
在 `CategoryPanel` 的析构函数中，通过将 `m_isInternalUpdating` 设为 `true` 并安全断开展开/折叠信号，确保在析构时没有任何 collapsed 信号污染 `AppConfig`：
- In `src/ui/CategoryPanel.h` (declare):
  ```cpp
  ~CategoryPanel() override;
  ```
- In `src/ui/CategoryPanel.cpp` (implement):
  ```cpp
...
- [ ] 模块/文件：`src/ui/CategoryPanel.h`
  - 涉及类/函数：`CategoryPanel::~CategoryPanel`
- [ ] 模块/文件：`src/ui/CategoryPanel.cpp`
  - 涉及类/函数：`CategoryPanel::~CategoryPanel`, `CategoryPanel::initUi`

**明确禁止越界修改的范围：**
- [ ] `CategoryModel::refresh` —— 不修改
- [ ] `CategoryRepo` —— 不修改
