...
## [2026-07-30] 取消托管库监控与自动导入底层资产剪切迁移重构

- 用户描述的现象/问题：
  1. NativeFolderWatcher (IOCP) 机制目前监控了托管库“ArcMeta.Library_[盘符]”文件夹，造成冗余的文件变更监听和资源浪费。
  2. 此前的“创建自动导入”功能被彻底根除。用户期望重新恢复自动导入功能，重整其底层自动同步与迁移的时序逻辑，使其感知到被监控外部目录项目变化时直接切入同盘符托管库中。
- 用户期望的结果：
  1. NativeFolderWatcher 彻底不再监控任何“ArcMeta.Library_[盘符]”托管库文件夹。
  2. 恢复初始版“创建自动导入”功能并将右键菜单、选项名称 and 对话框名称统一更名为“自动导入”。
  3. 重构“自动导入”监控变动底层逻辑：一旦感知到被监控的自定义目录下（例如 `Z:\测试`）有任何项目增、改等变动时，不进行原有的原地对账或原地元数据注册，而是直接以剪切/迁移方式，通过 AssetImporter 将变动项目物理搬移至该盘符对应的托管库（例如 `Z:\ArcMeta.Library_Z`）下，且无需弹出任何 FramelessMessageBox 问询。手动粘贴/拖拽等交互则保持当前已有逻辑不变。
- 本次任务边界：
  1. 物理修改 `src/core/CoreController.cpp` 及 `src/core/SystemBootstrapper.cpp`，彻底删除对托管库的 `addWatch` 监听注册。
  2. 物理恢复并重构 `CustomFolderImportDialog` 并更名为“自动导入”相关对话框。修改盘符栏 context menu 中“新建自动导入”与“解除监控”的选项文案和动作，统一为“自动导入”和相关解绑，确保 AppConfig 持久化存储与更新。
  3. 修改 `CoreController::CoreController` 对 IOCP `filesChanged` 的信号槽实现，当捕获的变动发生在任意自定义监控的“自动导入”目录下时，定位其顶级项目并通过 `AssetImporter` 进行剪切迁移；非监控目录下的项目变动维持原逻辑。
- 不在本次范围内的是：
  1. 修改 MFT / USN 底层核心扫描，修改底层 IOCP 事件泵模型。
  2. 改变手动粘贴或界面拖放导入等其他交互的提示确认与分流逻辑。
- 对应方案文档：Modification_Plan/Modification_Plan-10.md
