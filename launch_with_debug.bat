...
echo 🚀 正在启动ArcMeta.exe...
echo 📋 调试信息将输出到控制台
echo 📍 如果程序崩溃，转储文件将保存到: %LOCALAPPDATA%\CrashDumps
echo.

REM 启动程序并捕获输出
ArcMeta\ArcMeta.exe 2>&1 | tee debug_logs\startup_log_%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%%time:~6,2%.txt
