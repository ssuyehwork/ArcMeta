    ...
    : FramelessDialog("FERREX-META", parent) 
{
    ...
    // --- 2026-06-xx 物理分离样式：每个组件独立拥有边框与圆角 ---
    this->setStyleSheet(R"(
        QWidget#SearchContainer { background: transparent; border: none; }
        QWidget#DriveContainer { background: transparent; border: none; }
        ...
        #mainSearchEdit:focus, #extSearchEdit:focus { 
        ...
        QPushButton#searchIconButton:hover { background: #FFA500; border-color: #FFA500; } 
        QPushButton#searchIconButton:pressed { background: #CC6600; border-color: #CC6600; }
...
void ScanDialog::setupUi() {
    ...
    // 2026-06-xx 按照建议：将间距增加到 10，给组件之间留出物理切割的空隙
    ...
    searchBarLayout->addWidget(m_searchEdit, 1);
    ...
    searchBarLayout->addWidget(m_extEdit);

    m_searchBtn = new QPushButton();
    m_searchBtn->setObjectName("searchIconButton");
    m_searchBtn->setFixedSize(48, 36); 
    m_searchBtn->setCursor(Qt::PointingHandCursor);
    m_searchBtn->setIcon(UiHelper::getIcon("search", QColor("#000000"), 20));
    m_searchBtn->setIconSize(QSize(20, 20));
    connect(m_searchBtn, &QPushButton::clicked, this, &ScanDialog::onTriggerSearch);
    searchBarLayout->addWidget(m_searchBtn);

    searchRow->addLayout(searchBarLayout, 1);
