...
public:
    static bool writeAnchor(const std::wstring& drivePath, const std::wstring& volSerial) {
        ...
        if (!scchPath.empty() && (scchPath.back() != L'\\' && scchPath.back() != L'/')) scchPath += L'\\';
        scchPath += L".scch";
        ...
        std::string serial = QString::fromStdWString(volSerial).toStdString();
    ...
    static std::wstring readAnchor(const std::wstring& scchPath) {
        ...
        if (n == 1 && memcmp(anchor.magic, SCCH_MAGIC, 4) == 0) {
            return QString::fromStdString(anchor.vol_serial).toStdWString();
        }
        return L"";
    }
};
