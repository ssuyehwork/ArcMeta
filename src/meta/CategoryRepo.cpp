...
bool CategoryRepo::addItemToCategoryBatch(int categoryId, const std::vector<std::pair<std::string, std::wstring>>& items) { 
    ...
    // 按关联盘符数据库对 items 进行归类分组，防止对不同的 db 重复开启关闭事务 
    std::unordered_map<sqlite3*, std::vector<std::pair<std::string, std::wstring>>> dbGroup; 
    for (const auto& item : items) { 
        ...
        if (memDb) { 
            dbGroup[memDb].push_back({item.first, finalPath}); 
        } 
    } 
    ...
    for (auto& [db, dbItems] : dbGroup) { 
        ...
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) { 
            ...
            for (const auto& dbItem : dbItems) { 
                ...
                sqlite3_bind_text(stmt, 2, dbItem.first.c_str(), -1, SQLITE_TRANSIENT); 
                sqlite3_bind_text16(stmt, 3, dbItem.second.c_str(), -1, SQLITE_TRANSIENT); 
                ...
                // 暂时这里根据 Repo 缓存逻辑，可以用 getItemCategoryIds 结合， 
                // 但由于 addItemToCategoryBatch 会在事务中成批执行，我们统一在事务提交后 refreshMemoryCache 
                countAdded++; 
            } 
            sqlite3_finalize(stmt); 
            if (transOk) { 
    ...
    if (allOk) { 
        refreshMemoryCache(); 
        s_countsDirty.store(true); 
        // 大事务提交后，直接触发一次对账刷新和持久化信息增量 
        int totalVirtItems = static_cast<int>(items.size()); 
        s_uncategorizedCount.fetch_sub(totalVirtItems); 
        s_categorizedCount.fetch_add(totalVirtItems); 
        updatePersistentStat(STAT_CATEGORIZED, totalVirtItems); 
