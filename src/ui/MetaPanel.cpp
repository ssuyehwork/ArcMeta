...
void ElasticEdit::adjustHeight() {
    ...
    if (this->height() != newH) { setFixedHeight(newH); }
}

void ElasticEdit::resizeEvent(QResizeEvent* e) { QPlainTextEdit::resizeEvent(e); adjustHeight(); }
...
PaletteCapsule::PaletteCapsule(QWidget* parent) : QWidget(parent) {
    setFixedHeight(28); setMouseTracking(true); setCursor(Qt::PointingHandCursor);
}

void PaletteCapsule::setPalette(const QVector<QPair<QColor, float>>& palette) {
    m_palette = palette; updateGeometry(); update();
}
...
QSize PaletteCapsule::minimumSizeHint() const { return QSize(28, 28); }
...
void PaletteCapsule::paintEvent(QPaintEvent*) {
    ...
    painter.setPen(QPen(QColor("#4D4D4D"), 1)); painter.setBrush(QColor("#2E2E2E"));
    painter.drawRoundedRect(rect().adjusted(0, 0, -1, -1), 14, 14);
    for (int i = 0; i < totalDots; ++i) {
        int x = m_padding + (int)(i * (m_dotSize + currentSpacing));
        if (x + m_dotSize > availableW - m_padding + 2) break;
        QRect dotRect(x, (height() - m_dotSize) / 2, m_dotSize, m_dotSize);
        painter.setPen(Qt::NoPen); painter.setBrush(m_palette[i].first);
        painter.drawEllipse(dotRect.adjusted(1, 1, -1, -1));
        if (i == m_hoverIndex) {
            painter.setBrush(Qt::NoBrush); painter.setPen(QPen(Qt::white, 1.0));
...
void PaletteCapsule::mouseMoveEvent(QMouseEvent* event) {
    ...
    for (int i = 0; i < m_palette.size(); ++i) {
        ...
        if (dotRect.contains(event->pos())) { newHover = i; break; }
    }
...
void PaletteCapsule::mousePressEvent(QMouseEvent* event) {
    if (m_hoverIndex != -1) {
        QMenu menu(this); UiHelper::applyMenuStyle(&menu);
...
TagPill::TagPill(const QString& text, QWidget* parent) : QWidget(parent), m_text(text) {
    setProperty("tagText", text); setFixedHeight(22);
    QHBoxLayout* layout = new QHBoxLayout(this); layout->setContentsMargins(8, 0, 4, 0); layout->setSpacing(4);
    QLabel* lbl = new QLabel(text, this); lbl->setStyleSheet("color: #EEEEEE; font-size: 13px; border: none; background: transparent;");
    m_closeBtn = new QPushButton(this); m_closeBtn->setFixedSize(14, 14); m_closeBtn->setCursor(Qt::PointingHandCursor);
    m_closeBtn->setIcon(UiHelper::getIcon("close", QColor("#B0B0B0"), 12)); m_closeBtn->setIconSize(QSize(10, 10));
    ...
    layout->addWidget(lbl); layout->addWidget(m_closeBtn);
    connect(m_closeBtn, &QPushButton::clicked, [this]() { emit deleteRequested(m_text); });
    QFontMetrics fm(lbl->font()); setFixedWidth(fm.horizontalAdvance(text) + 30);
}

void TagPill::paintEvent(QPaintEvent*) {
    QPainter painter(this); painter.setRenderHint(QPainter::Antialiasing);
    painter.setBrush(QColor("#2B2B2B")); painter.setPen(QPen(QColor("#444444"), 1));
...
QSize FlowLayout::minimumSize() const {
    QSize size; for (QLayoutItem *item : itemList) size = size.expandedTo(item->minimumSize());
    size += QSize(2 * contentsMargins().top(), 2 * contentsMargins().top()); return size;
}
int FlowLayout::doLayout(const QRect &rect, bool testOnly) const {
    int left, top, right, bottom; getContentsMargins(&left, &top, &right, &bottom);
    QRect effectiveRect = rect.adjusted(+left, +top, -right, -bottom);
    int x = effectiveRect.x(), y = effectiveRect.y(), lineHeight = 0;
    for (QLayoutItem *item : itemList) {
        int spaceX = horizontalSpacing(), spaceY = verticalSpacing();
...
void StarRatingWidget::mousePressEvent(QMouseEvent* e) {
    e->accept(); int x = e->pos().x(), starSize = 18, spacing = 1, index = x / (starSize + spacing);
    if (index >= 0 && index < 5) { int newRating = index + 1; if (newRating == m_rating) newRating = 0; setRating(newRating); emit ratingChanged(newRating); }
}
...
void ColorPickerWidget::setColor(const std::wstring& name) {
    m_currentColor = name; if (m_colors.size() > 9) m_colors.resize(9);
    if (!name.empty() && name[0] == L'#') { QColor customColor(QString::fromStdWString(name)); if (customColor.isValid()) m_colors.push_back({name, customColor}); }
    setFixedSize((int)m_colors.size() * 24, 24); update();
}
...
void ColorPickerWidget::mousePressEvent(QMouseEvent* e) {
    e->accept(); int x = e->pos().x(), index = x / 24;
    if (index >= 0 && index < (int)m_colors.size()) { setColor(m_colors[index].name); emit colorChanged(m_colors[index].name); }
}
...
void MetaPanel::initUi() {
    ...
    QHBoxLayout* headerLayout = new QHBoxLayout(header); headerLayout->setContentsMargins(15, 0, 5, 0); headerLayout->setSpacing(5);
    ...
    m_container = new QWidget(m_scrollArea); m_containerLayout = new QVBoxLayout(m_container);
    m_containerLayout->setContentsMargins(10, 10, 10, 10); m_containerLayout->setSpacing(8);
    m_paletteCapsule = new PaletteCapsule(m_container); connect(m_paletteCapsule, &PaletteCapsule::colorSelected, this, &MetaPanel::searchByColor);
    QWidget* palWrapper = new QWidget(m_container); palWrapper->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    QHBoxLayout* palWrapperL = new QHBoxLayout(palWrapper); palWrapperL->setContentsMargins(0, 0, 0, 0); palWrapperL->addWidget(m_paletteCapsule); palWrapperL->addStretch();
    m_containerLayout->addWidget(palWrapper);
    m_nameEdit = new ElasticEdit(m_container); m_nameEdit->setPlaceholderText("文件名称...");
    m_nameEdit->setStyleSheet(QString("QPlainTextEdit { background: %1; border: 1px solid %2; border-radius: 4px; padding: 6px 10px; font-size: 13px; font-weight: bold; color: %3; }")
        .arg(Style::qssColor(Style::BackgroundDeep)).arg(Style::qssColor(Style::BorderColor)).arg(Style::qssColor(Style::TextMain)));
    m_nameEdit->installEventFilter(this); m_containerLayout->addWidget(m_nameEdit);
    m_noteEdit = new ElasticEdit(m_container); m_noteEdit->setPlaceholderText("添加备注说明...");
    ...
    m_noteEdit->installEventFilter(this); m_containerLayout->addWidget(m_noteEdit);
    m_linkEdit = new ElasticEdit(m_container); m_linkEdit->setPlaceholderText("添加链接...");
    ...
    m_linkEdit->installEventFilter(this); m_containerLayout->addWidget(m_linkEdit);
    QWidget* tagBox = new QWidget(m_container); QVBoxLayout* tagL = new QVBoxLayout(tagBox); tagL->setContentsMargins(0, 0, 0, 0); tagL->setSpacing(8);
    m_tagContainer = new QWidget(tagBox); m_tagFlowLayout = new FlowLayout(m_tagContainer, 0, 4, 4); tagL->addWidget(m_tagContainer);
    m_tagEdit = new QLineEdit(tagBox); m_tagEdit->setPlaceholderText("输入标签..."); m_tagEdit->setFixedHeight(24);
    ...
    connect(m_tagEdit, &QLineEdit::returnPressed, this, &MetaPanel::onTagAdded); tagL->addWidget(m_tagEdit); m_containerLayout->addWidget(tagBox);
    m_categoryEdit = new ElasticEdit(m_container); m_categoryEdit->setReadOnly(true);
    ...
    addInfoRow("类型", lblType); addInfoRow("大小", lblSize); addInfoRow("创建时间", lblCtime); addInfoRow("修改时间", lblMtime); addInfoRow("访问时间", lblAtime);
    addInfoRow("物理路径", lblPath); addInfoRow("加密状态", lblEncrypted);
    m_containerLayout->addStretch(1); m_scrollArea->setWidget(m_container); m_mainLayout->addWidget(m_scrollArea);
}

void MetaPanel::addInfoRow(const QString& label, QLabel*& valueLabel) {
    QWidget* row = new QWidget(m_container); QHBoxLayout* rl = new QHBoxLayout(row); rl->setContentsMargins(0, 2, 0, 2); rl->setSpacing(8);
    QLabel* kl = new QLabel(label, row); kl->setFixedWidth(80); kl->setStyleSheet("font-size: 13px; color: #888888;"); rl->addWidget(kl, 0, Qt::AlignTop);
    valueLabel = new QLabel("-", row); valueLabel->setWordWrap(true); valueLabel->setTextInteractionFlags(Qt::TextSelectableByMouse);
    valueLabel->setStyleSheet("font-size: 13px; color: #CCCCCC; line-height: 1.5;"); valueLabel->setAlignment(Qt::AlignLeft | Qt::AlignTop); rl->addWidget(valueLabel, 1);
...
void MetaPanel::onTagAdded() {
    ...
    if (!text.isEmpty()) {
        ...
        if (currentPath != "-" && !currentPath.isEmpty()) {
            std::wstring wPath = currentPath.toStdWString(); RuntimeMeta rm = MetadataManager::instance().getMeta(wPath);
...
void MetaPanel::resizeEvent(QResizeEvent* event) {
    ...
    if (maxW > 0) {
        m_nameEdit->setMaximumWidth(maxW); m_noteEdit->setMaximumWidth(maxW); m_linkEdit->setMaximumWidth(maxW); m_categoryEdit->setMaximumWidth(maxW);
...
void MetaPanel::updateInfo(const QString& n, const QString& t, const QString& s, const QString& ct, const QString& mt, const QString& at, const QString& p, bool e) {
    m_nameEdit->blockSignals(true); QFileInfo info(n); m_nameEdit->setPlainText(info.completeBaseName()); m_nameEdit->adjustHeight();
    m_nameEdit->setProperty("oldPath", p); m_nameEdit->setProperty("suffix", info.suffix()); m_nameEdit->blockSignals(false);
    ...
    if (p != "-" && !p.isEmpty()) {
        RuntimeMeta rm = MetadataManager::instance().getMeta(p.toStdWString()); setNote(rm.note); setURL(rm.url); setTags(rm.tags);
        QVector<QPair<QColor, float>> pal; for (const auto& entry : rm.palettes) { pal.append({entry.color, entry.ratio}); } setPalettes(pal);
    }
    if (m_container) m_container->adjustSize();
}

void MetaPanel::setRating(int rating) { emit metadataChanged(rating, L"__NO_CHANGE__"); }
void MetaPanel::setColor(const std::wstring& color) { emit metadataChanged(-1, color); }
...
void MetaPanel::setNote(const std::wstring& note) { 
    m_noteEdit->blockSignals(true); m_noteEdit->setPlainText(QString::fromStdWString(note)); m_noteEdit->adjustHeight(); m_noteEdit->blockSignals(false);
    if (m_container) m_container->adjustSize();
}
void MetaPanel::setURL(const std::wstring& url) { 
    m_linkEdit->blockSignals(true); m_linkEdit->setPlainText(QString::fromStdWString(url)); m_linkEdit->adjustHeight(); m_linkEdit->blockSignals(false);
    if (m_container) m_container->adjustSize();
}
void MetaPanel::setCategory(const QString& category) { 
    m_categoryEdit->blockSignals(true); m_categoryEdit->setPlainText(category); m_categoryEdit->adjustHeight(); m_categoryEdit->blockSignals(false);
    if (m_container) m_container->adjustSize();
}

void MetaPanel::setPalettes(const QVector<QPair<QColor, float>>& palette) {
    if (m_paletteCapsule) { m_paletteCapsule->setPalette(palette); }
    if (m_container) { m_container->adjustSize(); }
}
...
bool MetaPanel::eventFilter(QObject* watched, QEvent* event) {
    if (watched == m_noteEdit && event->type() == QEvent::FocusOut) {
        ...
        QString oldPath = m_nameEdit->property("oldPath").toString(), newName = m_nameEdit->toPlainText().trimmed();
        static const QString illegalChars = "\\/:*?\"<>|"; for (auto c : illegalChars) newName.remove(c); m_nameEdit->setPlainText(newName);
        ...
        if (!oldPath.isEmpty() && !newName.isEmpty()) {
            ...
            if (newName != oldInfo.completeBaseName()) {
                QString newPath = QDir::toNativeSeparators(oldInfo.absolutePath() + "/" + newName + (suffix.isEmpty() ? "" : "." + suffix));
                if (QFile::exists(newPath)) { m_nameEdit->setPlainText(oldInfo.completeBaseName()); return true; }
                if (QFile::rename(oldPath, newPath)) { MetadataManager::instance().renameItem(oldPath.toStdWString(), newPath.toStdWString()); lblPath->setText(newPath); m_nameEdit->setProperty("oldPath", newPath); }
                else { m_nameEdit->setPlainText(oldInfo.completeBaseName()); }
            }
        }
    }
