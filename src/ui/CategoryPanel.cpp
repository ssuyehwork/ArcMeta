...
void CategoryPanel::initUi() {
    ...
    connect(m_categoryTree, &DropTreeView::pathsDropped, this, [this](const QStringList& paths, const QModelIndex& proxyIndex) {
        ...
        // 理由：入库动作由且仅由 USN Journal 触发。此处委托给专门的物理迁移中枢。
        FileMigrationHandler::handleDroppedPaths(paths, targetCatId, this);
    });
...
bool FileMigrationHandler::handleDroppedPaths(const QStringList& paths, int targetCatId, QWidget* parent) {
    ...
    if (finalPaths.isEmpty()) return false;
    ...
    if (!managedRoot.isEmpty()) {
        ImportHelper::importPaths(finalPaths, managedRoot, parent);
        return true;
    }
    return false;
}
