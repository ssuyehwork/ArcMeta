...
void FilterPanel::rebuildGroups() {
    ...
    // ── 7. 链接 (Plan-30) ─────────────────────────
    {
        ...
        for (auto p : {FilterState::Yes, FilterState::No}) {
            ...
            QCheckBox* cb = addFilterRow(gl, label, 0);
            cb->blockSignals(true);
    ...
    // ── 9. 文件大小 (Plan-30) ─────────────────────────
    {
        ...
        QWidget* sizeRow = new QWidget(g);
        QHBoxLayout* hs = new QHBoxLayout(sizeRow);
        hs->setContentsMargins(8, 8, 8, 8);
        hs->setSpacing(6);

        QLineEdit* minEdit = new QLineEdit(sizeRow);
        QLineEdit* maxEdit = new QLineEdit(sizeRow);
        QComboBox* unitCombo = new QComboBox(sizeRow);
        unitCombo->addItems({"KB", "MB", "GB"});
        unitCombo->setCurrentIndex(1);

        auto sizeEditStyle = "QLineEdit { background: #2D2D2D; color: #EEE; border: 1px solid #444; border-radius: 4px; padding: 4px; font-size: 11px; }";
        ...
        // 优化 QComboBox 样式：更明显的三角形指示器
        QString arrowPath = UiHelper::getSvgTempFilePath("solid_down_triangle", QColor("#AAAAAA"));
        unitCombo->setStyleSheet(QString(
            "QComboBox { background: #2D2D2D; color: #EEEEEE; border: 1px solid #444444; border-radius: 4px; font-size: 11px; padding: 1px 25px 1px 5px; min-width: 50px; height: 24px; }"
            "QComboBox::drop-down { border: none; width: 20px; }"
            "QComboBox::down-arrow { image: url(%1); subcontrol-origin: padding; subcontrol-position: center right; right: 4px; width: 10px; height: 10px; }"
        ...
        QLabel* sep = new QLabel("-", sizeRow); sep->setStyleSheet("color: #AAA;"); hs->addWidget(sep);
        hs->addWidget(maxEdit);
        hs->addWidget(unitCombo);
        hs->addStretch();

        gl->addWidget(sizeRow);
    ...
    // ── 10. 图像比例 (Plan-29) ─────────────────────────
    {
        ...
        for (const auto& pair : ratios) {
            QCheckBox* cb = addFilterRow(gl, pair.second, 0);
            cb->blockSignals(true);
