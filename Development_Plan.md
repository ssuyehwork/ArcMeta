...
- 不在本次范围内的是：修改任何除了启动时导航栏白色边框样式修复之外的逻辑，不修改物理文件检索扫描对账逻辑，不修改数据库、MFT 扫描或其它无关 of UI 组件。
...
## [2026-07-29] 长期托管库极简重构与自动导入功能完好保持 —— Modification_Plan-2.md

- 用户描述的现象/问题：
  在原有的 `Modification_Plan-1.md` 方案中，设计方案将托管库重构与自定义监控自动导入（临时工作区）的叙述交织在一起。为了保证代码修改的高内聚与安全性，需要只对主托管库 `ArcMeta.Library_[盘符]` 相关的代码及数据层实施彻底重构（.arc 物理资产包封装、侧边栏一等公民、添加日期元数据及排序过滤），而原有的外部“创建自动导入”原地监控与原位索引的底层逻辑必须保持 100% 完好不变。
- 用户期望的结果：
  产出极度纯净、无任何越界和干扰的独立重构方案文档 `Modification_Plan-2.md`，精确界定主托管库的重构范围、不涉及修改的自动导入范围，并在物理落盘、侧边栏逻辑、排序扩展、UI 过滤等层面给出完美的设计。
- 本次任务边界：
  1. 梳理并明确主托管库与外部自动导入的核心模式边界，建立 100% 物理红线隔离。
  2. 针对主托管库规划 `.arc` 物理包平铺格式、13位 Base36 ID 算法、多梯队 `_thumbnail.png` 异步原子生成时序。
  3. 将 `ArcMeta.Library_[盘符]` 的 `parentId` 调整为 `0`，废除多余“我的分类”虚拟外壳。
  4. 升级 SQLite `metadata` 数据库，在 `loadDb` 中追加 `added_at` 的平滑迁移 DDL，并扩展排序。
  5. 产出方案文档 `Modification_Plan/Modification_Plan-2.md`，并在文档中对照 `AGENTS.md` 约束（如绝对不提及 AGENTS.md、第 7 节合规检查写入具体内容）。
- 不在本次范围内的是：
  修改或重构“创建自动导入”对应的外部原地监控代码逻辑（如不改动 `CustomMonitoredFolders`）；修改或重构 MFT / IOCP 底盘文件引擎及 `QuickLookWindow` 预览组件；当前在分析师模式下修改任何代码文件。
